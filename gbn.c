...
int B_expectseq; //expected sequence number
int cur_windowsize;
struct pkt pack[WINDOW_SIZE];
struct pkt B_packet;
//struct msg buffer[DATA_LENGTH];
...
int check_checksum(struct pkt p){
	//comparing the checksum to the computed checksum
	return(p.checksum == calc_checksum(p));
}

void A_process(struct msg m){
	struct pkt p;
	memcpy(p.payload, m.data, DATA_LENGTH);
	p.seqnum = A_nextseq;
	p.acknum = 0;
	p.checksum = calc_checksum(p);
	//store the packet in the window
	pack[cur_windowsize] = p;
	cur_windowsize++;
	//send the packet to layer 3
	tolayer3((int)'A', p);
	//set timer when first packet in the window has being send out
	if(A_base == A_nextseq){
		if(getTimer((int)'A')){
			stopTimer((int)'A');
		}
		startTimer((int)'A', (float)1.0);
	}
	//increse the sequence number
	A_nextseq++;
}
/* called from layer 5, passed the data to be sent to other side */
void A_output(struct msg message) {
	//If the window is not full, buffer is empty, and receive a non-empty message
	//ignore the cases of empty buffer, or empty message, or both of them. 
	if(cur_windowsize >=0 && cur_windowsize< WINDOW_SIZE && message.data[0] != 0){
		A_process(message);
	}
	//TODO: CHECK buffer  
}
...
void A_input(struct pkt packet) {
	if(check_checksum(packet) == 1 && packet.acknum <= A_base + WINDOW_SIZE){
		while(pack[0].seqnum <= packet.acknum && pack[0].seqnum != 0 && cur_windowsize >0){
			...
			memset(&pack[cur_windowsize -1],0, sizeof(struct pkt));
			cur_windowsize--;
...
void B_input(struct pkt packet) {
	if(check_checksum(packet)){
		...
		memcpy(new_packet.payload, packet.payload,DATA_LENGTH);
		new_packet.checksum = calc_checksum(new_packet);
		B_packet = new_packet;
		tolayer3((int)'B', new_packet);
		B_expectseq++;	
	}
	else{
		if(B_expectseq > 1){
